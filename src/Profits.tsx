    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-950/80 via-slate-900 to-slate-900 light:from-emerald-50 light:to-white border border-emerald-500/30 light:border-emerald-300 p-6 shadow-xl"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <DollarSign className="w-24 h-24 text-emerald-400 light:text-emerald-600" />
          </div>
          ...
          <div className="text-[11px] text-slate-400 light:text-slate-600 space-y-1 pt-2 border-t border-white/5 light:border-slate-200">
            <div className="flex justify-between">
              <span>{t("grossRevenue")}</span>
              <span className="font-semibold text-slate-200 light:text-slate-900">EGP {todayProfit.toFixed(2)}</span>
            </div>
            {todayMaintenanceExpense > 0 && (
              <div className="flex justify-between text-red-400 light:text-red-600">
                <span>{t("equipmentRepairs")}</span>
                <span className="font-semibold">- EGP {todayMaintenanceExpense.toFixed(2)}</span>
              </div>
            )}
            <div className="flex justify-between text-emerald-300 light:text-emerald-700 font-bold">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-950/80 via-slate-900 to-slate-900 light:from-blue-50 light:to-white border border-blue-500/30 light:border-blue-300 p-6 shadow-xl"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <Calendar className="w-24 h-24 text-blue-400 light:text-blue-600" />
          </div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-400 light:text-blue-700 flex items-center gap-1.5">
          ...
          <div className="text-[11px] text-slate-400 light:text-slate-600 space-y-1 pt-2 border-t border-white/5 light:border-slate-200">
            <div className="flex justify-between">
              <span>{t("grossRevenue")}</span>
              <span className="font-semibold text-slate-200 light:text-slate-900">EGP {monthProfit.toFixed(2)}</span>
            </div>
            ...
            <div className="flex justify-between text-blue-300 light:text-blue-700 font-bold">
              <span>{t("completedSessionsCount")}</span>
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-950/80 via-slate-900 to-slate-900 light:from-purple-50 light:to-white border border-purple-500/30 light:border-purple-300 p-6 shadow-xl"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <Award className="w-24 h-24 text-purple-400 light:text-purple-600" />
          </div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold uppercase tracking-wider text-purple-400 light:text-purple-700 flex items-center gap-1.5">
          ...
          <div className="text-[11px] text-slate-400 light:text-slate-600 space-y-1 pt-2 border-t border-white/5 light:border-slate-200">
            <div className="flex justify-between">
              <span>{t("grossRevenue")}</span>
              <span className="font-semibold text-slate-200 light:text-slate-900">EGP {totalProfitAllTime.toFixed(2)}</span>
            </div>
            ...
            <div className="flex justify-between text-purple-300 light:text-purple-700 font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-slate-900/90 light:bg-white border border-white/10 light:border-slate-300 rounded-2xl p-5 shadow-xl">
            <div className="flex items-center justify-between pb-4 border-b border-white/10 light:border-slate-200 mb-4">
              <div>
                <h2 className="text-lg font-bold text-white light:text-slate-900 flex items-center gap-2">
                  <Calendar className="w-5 h-5 text-emerald-400 light:text-emerald-600" />
              ...
              <div className="text-xs font-medium text-slate-400 light:text-slate-600 bg-slate-800 light:bg-slate-100 px-3 py-1 rounded-lg border border-white/5 light:border-slate-200">
                {dailyHistory.length} {t("activeDays")}
              </div>
            </div>

            {loading ? (
              <div className="py-12 text-center text-slate-400 light:text-slate-500">{t("loadingProfitHistory")}</div>
            ) : dailyHistory.length === 0 ? (
              <div className="py-12 text-center text-slate-500 light:text-slate-500 text-sm">
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-xl transition-all overflow-hidden ${
                        isToday
                          ? "border-emerald-500/40 light:border-emerald-300 bg-emerald-950/20 light:bg-emerald-50"
                          : "border-white/10 light:border-slate-200 bg-slate-800/40 light:bg-slate-50 hover:bg-slate-800/70 light:hover:bg-slate-100"
                      }`}
                    >
                      {/* Day Header Row */}
                      <div
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-lg flex items-center justify-center font-bold text-xs ${
                              isToday
                                ? "bg-emerald-500/20 light:bg-emerald-100 text-emerald-300 light:text-emerald-800 border border-emerald-500/30 light:border-emerald-300"
                                : "bg-slate-700/60 light:bg-slate-200 text-slate-300 light:text-slate-700 border border-white/5 light:border-slate-300"
                            }`}
                          >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            <div className="text-base font-extrabold text-emerald-400 light:text-emerald-700">
                              EGP {dayGroup.total.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </div>
                          </div>
                          <button
                            type="button"
                            className="p-1 rounded-md text-slate-400 light:text-slate-600 hover:text-white light:hover:text-slate-900 hover:bg-slate-700 light:hover:bg-slate-200"
                          >
                      ...
                      <AnimatePresence>
                        {isExpanded && (
                          <motion.div
                            ...
                            transition={{ duration: 0.2 }}
                            className="border-t border-white/10 light:border-slate-200 bg-slate-900/60 light:bg-white px-4 py-3"
                          >
                            <div className="overflow-x-auto">
                              <table className="w-full text-xs text-left text-slate-300 light:text-slate-900">
                                <thead>
                                  <tr className="border-b border-white/10 light:border-slate-200 text-slate-400 light:text-slate-700 font-semibold">
                                ...
                                <tbody className="divide-y divide-white/5 light:divide-slate-200">
                                  {dayGroup.items.map((item) => (
                                    <tr key={item.id} className="hover:bg-white/5 light:hover:bg-slate-100 transition-colors">
                                      <td className="py-2 px-2 font-medium text-white light:text-slate-900">
                                        {item.CustomerName || "—"}
                                      </td>
                                      <td className="py-2 px-2 text-slate-300 light:text-slate-700">
                                        {item.VendorName || ""} {item.ModelName || ""} ({item.Device_Type || "—"})
                                      </td>
                                      <td className="py-2 px-2 text-slate-400 light:text-slate-500 max-w-[150px] truncate">
                                        {item.issue || item.Notes || "—"}
                                      </td>
                                      <td className="py-2 px-2 text-blue-400 light:text-blue-700 font-medium">
                                        {item.DoneBy || "—"}
                                      </td>
                                      <td className="py-2 px-2 text-right font-bold text-emerald-400 light:text-emerald-700">
                                      ...
                                      <td className="py-2 px-2 text-center">
                                        <button
                                          ...
                                          className="px-2 py-1 text-[11px] font-bold rounded-lg bg-emerald-500/20 light:bg-emerald-100 hover:bg-emerald-500/30 light:hover:bg-emerald-200 text-emerald-300 light:text-emerald-800 border border-emerald-500/30 light:border-emerald-300 inline-flex items-center gap-1 transition-colors"
                                          title="View / Print Receipt History"
                                        >
                                          <Receipt className="w-3.5 h-3.5 text-emerald-400 light:text-emerald-700" />
        ...
        <div className="space-y-4">
          <div className="bg-slate-900/90 light:bg-white border border-white/10 light:border-slate-300 rounded-2xl p-5 shadow-xl">
            <div className="pb-4 border-b border-white/10 light:border-slate-200 mb-4">
              <h2 className="text-lg font-bold text-white light:text-slate-900 flex items-center gap-2">
                <User className="w-5 h-5 text-blue-400 light:text-blue-600" />
            ...
            {techBreakdown.length === 0 ? (
              <div className="py-8 text-center text-slate-500 light:text-slate-500 text-sm">
                {t("noTechDataMonth")}
              </div>
            ) : (
              <div className="space-y-3">
                {techBreakdown.map((tech, idx) => {
                  ...
                  return (
                    <div
                      key={tech.name}
                      className="p-3.5 rounded-xl bg-slate-800/40 light:bg-slate-50 border border-white/5 light:border-slate-200 hover:border-white/10 transition-colors"
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-full bg-blue-500/20 light:bg-blue-100 text-blue-300 light:text-blue-800 text-[10px] font-bold flex items-center justify-center border border-blue-500/30 light:border-blue-300">
                            #{idx + 1}
                          </span>
                          <span className="text-sm font-semibold text-white light:text-slate-900">{tech.name}</span>
                        </div>
                        <span className="text-sm font-extrabold text-emerald-400 light:text-emerald-700">
                          EGP {tech.total.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                      </div>

                      {/* Progress Bar */}
                      <div className="w-full bg-slate-700/50 light:bg-slate-200 h-2 rounded-full overflow-hidden my-2">
                        <div
