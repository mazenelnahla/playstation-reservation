...
import { loadStations, Station } from "../../DataHandle/Station";
...
export default function NewSessionDialog({
  ...
  const [vendorNames, setVendorNames] = useState<Station[]>([]);
  ...
  useEffect(() => {
    if (open) {
      setErrorMsg("");
      Promise.all([loadStations(), fetchMaintenanceLogs(), load()])
        .then(([vNames, mLogs, records]) => {
          ...
          const activeSet = new Set(
            (records || [])
              .filter((r: SessionRecord) => {
                const ended = r.endTime !== undefined ? r.endTime : r.Date_out;
                return !ended || ended.trim() === "";
              })
              .map((r: SessionRecord) => r.stationName || r.VendorName || "")
          );
  ...
  async function addItem(e: React.FormEvent<HTMLFormElement>) {
    ...
    const customerName = String(form.get("CustomerName") || "Guest Customer");
    const customerPhone = String(form.get("CustomerPhoneNumber") || "");
    const sessionNotes = String(form.get("issue") || "Gaming Session");
    ...
    const recordData = {
      ...
      stationName: targetVendor,
      gameType: bookedHours,
      sessionNotes,
      hourlyRate: String(calculatedPrice),
      staffMember: loggedInEmployee,
      endTime: "",
      notes,
    ...
    try {
      ...
      console.error("[NewSessionDialog addItem] failed", err);
    } finally {
      setSubmitting(false);
    }
  }
