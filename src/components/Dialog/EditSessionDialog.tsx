...
import { SessionRecord } from "../../DataHandle/storage";
import { useLanguage } from "../../context/LanguageContext";

type RecordItem = SessionRecord;
...
export default function EditSessionDialog({
  ...
  const [customerName, setCustomerName] = useState(editing.customerName || editing.CustomerName || "");
  const [customerPhoneNumber, setCustomerPhoneNumber] = useState(editing.customerPhone || editing.CustomerPhoneNumber || "");
  const [vendorName, setVendorName] = useState(editing.stationName || editing.VendorName || "");
  const [deviceType, setDeviceType] = useState(editing.stationType || editing.Device_Type || CATEGORY_OPTIONS[0]);
  const [modelName, setModelName] = useState(editing.gameType || editing.ModelName || "");
  const [issue, setIssue] = useState(editing.sessionNotes || editing.issue || "");
  const [maintinancePrice, setMaintinancePrice] = useState(editing.hourlyRate || editing.MaintinancePrice || "");
  const [doneBy, setDoneBy] = useState(editing.staffMember || editing.DoneBy || (users[0]?.name || "Staff"));
  const [dateIn, setDateIn] = useState(editing.startTime || editing.Date_in || "");
  const [dateOut, setDateOut] = useState(editing.endTime || editing.Date_out || "");
  const [notes, setNotes] = useState(editing.notes || editing.Notes || "");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updated: RecordItem = {
      ...
      stationType: deviceType,
      stationName: vendorName,
      gameType: modelName,
      sessionNotes: issue,
      hourlyRate: maintinancePrice,
      endTime: dateOut,
      staffMember: doneBy,
      notes: notes,
